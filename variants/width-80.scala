 ...
 *
 * The same three steps over a declared schema of 80 attributes: reading,
...
object Width80 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val spark = SparkSession.builder().master("local[*]").appName("Width80").getOrCreate()
    spark.sparkContext.setLogLevel("WARN")

    val d0 = Loader.json[Document, W80](spark, path)
    val d1 = add[Document, "declared_width" :: HNil](d0)(_ => 80)
