 ...
 *
 * The projection keeps the attribute nested inside nutriments, and
 * announces the result as relational.
 *
 * Expected: rejected at compile time.
 */
object Pf2BadTransfo {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val spark = SparkSession.builder().master("local[*]").appName("Pf2BadTransfo").getOrCreate()
    ...
    val d7 = project[Relation, KeepNested](d6)
    Sink.typed(d7, Sink.scratch(spark, "Pf2BadTransfo"))
