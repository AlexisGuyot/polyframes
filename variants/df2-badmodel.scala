 ...
 *
 * The nested attribute the join carried across is still there when the
 * relational sink is reached.
 *
...
object Df2BadModel {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val spark = SparkSession.builder().master("local[*]").appName("Df2BadModel").getOrCreate()
    ...
    Sink.untyped(d6, Sink.scratch(spark, "Df2BadModel"))
