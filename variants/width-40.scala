 ...
 *
 * The same three steps over a declared schema of 40 attributes: reading,
...
object Width40 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val spark = SparkSession.builder().master("local[*]").appName("Width40").getOrCreate()
    spark.sparkContext.setLogLevel("WARN")

    val d0 = Loader.json[Document, W40](spark, path)
    val d1 = add[Document, "declared_width" :: HNil](d0)(_ => 40)
