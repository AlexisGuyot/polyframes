...
/**
 * DataFrames, an attribute absent from the schema, on the pipeline that
...
object Df2NoAttr {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val spark = SparkSession.builder().master("local[*]").appName("Df2NoAttr").getOrCreate()
    ...
    val d7 = d6.select("barcode", "grade_labell")
    Sink.untyped(d7, Sink.scratch(spark, "Df2NoAttr"))
