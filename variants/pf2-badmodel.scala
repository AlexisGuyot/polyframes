 ...
 *
 * The data is still a document when it reaches the relational sink: the
 * join carried the nested attribute across, and nothing has flattened it.
 *
...
object Pf2BadModel {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val spark = SparkSession.builder().master("local[*]").appName("Pf2BadModel").getOrCreate()
    ...
    Sink.typed(d6, Sink.scratch(spark, "Pf2BadModel"))
