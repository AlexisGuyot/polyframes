 ...
 *
 * The same three steps over a declared schema of 10 attributes: reading,
...
object Width10 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val spark = SparkSession.builder().master("local[*]").appName("Width10").getOrCreate()
    spark.sparkContext.setLogLevel("WARN")

    val d0 = Loader.json[Document, W10](spark, path)
    val d1 = add[Document, "declared_width" :: HNil](d0)(_ => 10)
