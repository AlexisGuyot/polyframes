 ...
 *
 * The same three steps over a declared schema of 20 attributes: reading,
...
object Width20 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val spark = SparkSession.builder().master("local[*]").appName("Width20").getOrCreate()
    spark.sparkContext.setLogLevel("WARN")

    val d0 = Loader.json[Document, W20](spark, path)
    val d1 = add[Document, "declared_width" :: HNil](d0)(_ => 20)
